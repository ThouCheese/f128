//! A crate for highly accurate computation, using 128 bits to represent a single floating
//! point number. Relies on the recently added `u128` to perform fixed point fixed point
//! operations which are then used to allow floating point operations which twice the accuracy
//! of `f64`, and values which are up to `1.1579e77` times as high.
//! 
//! # Basic Usage
//! Since the `f128` is not a primitive type of Rust, we can write things like `4 as f128` or 
//! `4f128`. Instead, this crate offers a `From` implementation for various numeric types.
//! ```
//! f128::from(28usize);
//! f128::from(-13i32);
//! f128::from(45e18f64);
//! ```
//! After that the `f128` behaves exactly like normal numbers, but with much greater accuracy.
//! ```
//! // imports omitted
//! let number1 = f128::from(12);
//! let number2 = f128::from(thread_rng().gen_range(3.0, 10000.0));
//! println!(number1 + number2.sin() / number1.sqrt());
//! ```
//! # Some benchmarks
//! Since the emulation of floating point operations can be expensive, it is good to keep in
//! mind the cost of each operation. With that in mind I provide a list of the benchmarks for 
//! your entertainment. As always with benchmarks: results may vary! `git clone` and `cargo
//! bench` on your own machine for a better indication!
//! ```
//! [me@my_computer f128]$ cargo bench
//!    Compiling f128 v0.1.0 (file:///home/me/code/rust/f128)                         
//!     Finished release [optimized] target(s) in 1.80s
//!      Running target/release/deps/f128-a9c0a4a9ffc0d5c8
//! 
//! running 16 tests
//! test tests::add_sub ... ignored
//! test tests::comparison ... ignored
//! test tests::floor ... ignored
//! test tests::mul_div ... ignored
//! test tests::sqrt ... ignored
//! test benches::add      ... bench:          14 ns/iter (+/- 0)
//! test benches::cos      ... bench:         313 ns/iter (+/- 1)
//! test benches::div      ... bench:         108 ns/iter (+/- 0)
//! test benches::from_f64 ... bench:          13 ns/iter (+/- 0)
//! test benches::from_i32 ... bench:          11 ns/iter (+/- 0)
//! test benches::mul      ... bench:           0 ns/iter (+/- 0)
//! test benches::neg      ... bench:           0 ns/iter (+/- 0)
//! test benches::ord      ... bench:           0 ns/iter (+/- 0)
//! test benches::sin      ... bench:         310 ns/iter (+/- 1)
//! test benches::sqrt     ... bench:         417 ns/iter (+/- 12)
//! test benches::sub      ... bench:          15 ns/iter (+/- 0)
//! 
//! test result: ok. 0 passed; 0 failed; 5 ignored; 11 measured; 0 filtered out
//! ```
...
impl f128 {
    ...
    pub fn sin(self) -> f128 {
        ...
        let frac = (5..LENGTH).step_by(4)
            .fold(num, |result, i| result + fixp_mul(powers[i/2], f128::inv_fact(i)));
        // now do the negative terms
        let frac = (3..LENGTH).step_by(4)
            .fold(frac, |result, i| result - fixp_mul(powers[i/2], f128::inv_fact(i)));
        if frac == 0 {
            return ZERO
        }
    ...
    pub fn sqrt(self) -> f128 {
        ...
        (0..5).fold(init, 
            |result, _| result * (three - self * result * result) * half
        ).with_sign(true) * self
    }
...
impl From<f64> for f128{
    fn from(number: f64) -> f128 {
        ...
        let frac = (number as f64 / 2f64.powf(exp as f64) * 
            TWO_TO_THE_112 as f64) as u128;
...
impl From<i32> for f128 {
    fn from(number: i32) -> f128 {
        ...
        let frac = number as u128 * TWO_TO_THE_112 / 
            2i32.pow(exp as u32) as u128 - TWO_TO_THE_112;
...
impl From<usize> for f128 {
    fn from(number: usize) -> f128 {
        ...
        let frac = TWO_TO_THE_112 / 
            2usize.pow(exp as u32) as u128 * number as u128 - TWO_TO_THE_112;
...
impl std::fmt::Display for f128 {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        ...
        write!(f, "{}", (TWO_TO_THE_112 + self.frac()) as f64 / TWO_TO_THE_112 as f64 * 
...
impl std::fmt::Debug for f128 {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        ...
        write!(f, "{}*2^{}", (TWO_TO_THE_112 + self.frac()) as f64 / TWO_TO_THE_112 as f64,
            self.exp())
    }
}
...
fn fixp_mul(a: u128, b: u128) -> u128 {
    let (a_int, a_float): (u128, u128) = 
        (a >> 112, a & 0x0000_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF);
    let (b_int, b_float): (u128, u128) = 
        (b >> 112, b & 0x0000_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF);
    (a_int * b_int << 112) + a_int * b_float + b_int * a_float + 
...
impl std::ops::Mul for f128 {
    ...
    fn mul(self, other: f128) -> f128 {
        ...
        let frac = floating_point_mul_112(self.frac(), other.frac()) + 
            self.frac() + other.frac() + TWO_TO_THE_112;
...
impl std::ops::Add for f128 {
    ...
    fn add(self, other: f128) -> f128 {
        ...
        let exp_diff = ((great.0 & 0x7FFF_0000_0000_0000_0000_0000_0000_0000) - 
                        (small.0 & 0x7FFF_0000_0000_0000_0000_0000_0000_0000)) >> 112;
...
mod tests {
    ...
    #[test]
    fn comparison() {
        let num1_generator: f64 = thread_rng().gen_range(-10000.0, 10000.0);
        let num1 = f128::from(num1_generator);
        let num2 = f128::from(thread_rng().gen_range(num1_generator, 10000.0));
    ...
    #[test]
    fn mul_div() {
        // division is a approximation, so we have to be lenient in our tests, and ignore the
        // last two bits (there are two operations, so the max error is 2, which is 10 in 
        // binary, so we need two bits)
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        let num2 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        print!("product of {} and {} is ", num1, num2);
        println!("{}", num1 * num2);
        assert_eq!(num2.0 & 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFC, 
                  (num1 * num2 / num1).0 & 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFC);
        assert_eq!(num1.0 & 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFC, 
                  (num2 * num1 / num2).0 & 0xFFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFF_FFFC);
    }
    ...
    fn add_sub() {
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        let num2 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        print!("sum of {} and {} is ", num1, num2);
        println!("{}", num1 + num2);
        assert_eq!(num2, num1 + num2 - num1);
        assert_eq!(num1, num2 + num1 - num2);
    }

    #[test]
    fn sqrt() {
        let num1 = f128::from(thread_rng().gen_range(0.0, 10000.0));
        print!("the square root of {} is ", num1);
        let sqrt = num1.sqrt();
        println!("{}", sqrt);
        assert_eq!(num1, sqrt * sqrt);
    }

    #[test]
    fn floor() {
        let num1 = f128::from(thread_rng().gen_range(0.0, 10000.0));
        print!("the floor of {} is ", num1);
        let floor = num1.floor();
        println!("{}", floor);
        assert!(num1 >= floor);
        assert!(num1 <= floor + ONE);
    }
}
...
mod benches {
    ...
    fn div(b: &mut Bencher) {
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        let num2 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        println!("dividing {} and {}", num1, num2);
        b.iter(|| num1 / num2); 
    }
    ...
    fn sub(b: &mut Bencher) {
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        let num2 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        println!("subtracting {} and {}", num1, num2);
        b.iter(|| num1 - num2); 
    }

    #[bench]
    fn ord(b: &mut Bencher) {
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        let num2 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        println!("comparing {} and {}", num1, num2);
        b.iter(|| num1 < num2); 
    ...
    #[bench]
    fn neg(b: &mut Bencher) {
        let num1 = f128::from(thread_rng().gen_range(-10000.0, 10000.0));
        println!("negating {}", num1);
        b.iter(|| -num1);
    }

    #[bench]
    fn from_i32(b: &mut Bencher) {
        let num1: i32 = thread_rng().gen_range(-10000, 10000);
        println!("converting {}", num1);
        b.iter(|| f128::from(num1));
    }

    #[bench]
    fn from_f64(b: &mut Bencher) {
        let num1: f64 = thread_rng().gen_range(-10000.0, 10000.0);
        println!("converting {}", num1);
        b.iter(|| f128::from(num1)); 
    }
